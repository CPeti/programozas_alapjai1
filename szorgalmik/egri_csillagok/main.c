...
#include <stdbool.h>
#include <string.h>
typedef enum hangrend{mely, magas, vegyes} hangrend;

hangrend milyen_hangrend(char szo[]){
    bool van_magas_maganhangzo = false;
    char magas_maganhanzgzok[2] = {'e', 'i'};
    for (int j = 0; j < 2; j++){
        if (strchr(szo, magas_maganhanzgzok[j]))
            van_magas_maganhangzo = true;
    }

    bool van_mely_maganhangzo = false;
    char mely_maganhanzgzok[3] = {'a', 'u', 'o'};
    for (int j = 0; j < 3; j++){
        if (strchr(szo, mely_maganhanzgzok[j]))
            van_mely_maganhangzo = true;
    }

    if (van_magas_maganhangzo && van_mely_maganhangzo)
        return vegyes;
    if (van_magas_maganhangzo)
        return magas;
    else
        return mely;
...
typedef enum mh{nincs, egyfele, tobbfele} mh;

mh milyen_mh(char szo[]){
    char mhtomb[] = {'a', 'e', 'i', 'o', 'u'};
    int tipus = 0;
    for (int j = 0; j < 3; j++){
        if (strchr(szo, mhtomb[j])){
            tipus++;
            if(tipus >= 2)
                return tobbfele;
        }
...
void elhagy(char szo[], char elhagyott[]){
    for(int i = 1; szo[i+1] != '\0'; i++){
        if (szo[i+1] != '\0')
            elhagyott[i-1] = szo[i];
        else
            elhagyott[i-1] = '\0';
...
int main()
{
    ...
    for (int i = 0; strcmp(szavak[i], "") != 0; i++){
        if (strstr(szavak[i], keresett) != NULL)
            printf("%s ", szavak[i]);

        hangrend vizsgalt = milyen_hangrend(szavak[i]);
        if (vizsgalt == magas)
            nmagas++;
        else if (vizsgalt == mely)
            nmely++;
        else if (vizsgalt == vegyes)
            nvegyes++;
    }
    printf("\n%d magas, %d mely, %d vegyes szo van\n", nmagas, nmely, nvegyes);
    printf("\n");


    printf("vizsgalt szo: ");
    scanf("%s", keresett);
    mh vizsgalt = milyen_mh(keresett);
        if (vizsgalt == nincs)
            printf("nem tartalmaz maganhangzokat\n");
        else if (vizsgalt == tobbfele)
            printf("kulonfele maganhangzokat tartalmaz\n");
        else if (vizsgalt == egyfele)
    ...
    for (int i = 0; strcmp(szavak[i], "") != 0; i++){
        char elhagyott[16];
        elhagy(szavak[i], elhagyott);
        for (int j = 0; strcmp(szavak[j], "") != 0; j++){
            if (strcmp(szavak[j], elhagyott) == 0){
                printf("%s - %s\n", szavak[i], elhagyott);
            }
        }
    }
